...
livewall_path="$HOME/Backgrounds/Live"
...
if [ "$XDG_SESSION_TYPE" = "wayland" ]; then
  killall awww-daemon mpvpaper livewall_auto_pause.sh
  mpvpaper eDP-1 -fo "input-ipc-server=/tmp/mpv-socket no-audio loop no-config" "$HOME/Backgrounds/Live/active"
